...
describe("validateFlightBody", () => {
  ...
  it("ошибка если номер рейса в неверном формате", () => {
    expect(validateFlightBody({ ...validBody, flight_number: "abc123" })).toBe(
      "Номер рейса: формат XX-NNN (например SU-101, U6-205)",
    );
  });

  it("ошибка если откуда и куда совпадают", () => {
    expect(
      validateFlightBody({ ...validBody, destination: "Москва (SVO)" }),
    ).toBe("Откуда и куда не могут совпадать");
  });

  it("ошибка если время прилёта раньше вылета", () => {
    expect(
      validateFlightBody({ ...validBody, arrival_time: hoursFromNow(1) }),
  ...
  it("ошибка если время прилёта равно времени вылета", () => {
    expect(
      validateFlightBody({
        ...validBody,
        arrival_time: validBody.departure_time,
      }),
    ).toBe("Время прилёта должно быть позже времени вылета");
  });

  it("ошибка если название города слишком короткое", () => {
    expect(validateFlightBody({ ...validBody, origin: "А" })).toBe(
      "Откуда: от 2 до 100 символов",
    );
  });

  it("ошибка если название пункта назначения слишком короткое", () => {
    expect(validateFlightBody({ ...validBody, destination: "Я" })).toBe(
      "Куда: от 2 до 100 символов",
    );
  });

  it("ошибка если дата вылета невалидна", () => {
    expect(
      validateFlightBody({ ...validBody, departure_time: "не-дата" }),
