...
async function main() {
  ...
  await prisma.flight.upsert({
    ...
    update: {},
    create: {
      flightNumber: 'SU-101', origin: 'Москва', destination: 'Санкт-Петербург',
      departureTime: new Date(now.getTime() + 2 * 3600000),
      arrivalTime: new Date(now.getTime() + 3.5 * 3600000),
      gateId: gates[0].id, status: 'SCHEDULED', createdById: admin.id
    }
  })
  await prisma.flight.upsert({
    ...
    update: {},
    create: {
      flightNumber: 'U6-205', origin: 'Екатеринбург', destination: 'Москва',
      departureTime: new Date(now.getTime() + 4 * 3600000),
      arrivalTime: new Date(now.getTime() + 6 * 3600000),
      gateId: gates[1].id, status: 'BOARDING', createdById: dispatcher.id
    }
  })
  await prisma.flight.upsert({
    ...
    update: {},
    create: {
      flightNumber: 'DP-301', origin: 'Казань', destination: 'Сочи',
      departureTime: new Date(now.getTime() + 1 * 3600000),
      arrivalTime: new Date(now.getTime() + 4 * 3600000),
      gateId: gates[2].id, status: 'DELAYED', createdById: dispatcher.id
    }
  })
